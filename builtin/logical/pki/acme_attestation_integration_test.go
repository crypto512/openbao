...
// TestTPMAttestationAllowlist tests TPM identifier allowlist functionality
func TestTPMAttestationAllowlist(t *testing.T) {
	// Create two simulated TPMs with different permanent identifiers
	tpm1 := NewSimulatedTPM(t, "TPM-ALLOWED-001", "TestManufacturer")
	tpm2 := NewSimulatedTPM(t, "TPM-NOT-ALLOWED-002", "TestManufacturer")

	keyAuthorization := "test-key-auth"

	// Generate a test key to certify
	testKey, err := rsa.GenerateKey(rand.Reader, 2048)
	require.NoError(t, err)

	// Test 1: Allowlist with allowed TPM should succeed
	t.Run("AllowedTPMSucceeds", func(t *testing.T) {
		attObj := tpm1.GenerateAttestation(t, keyAuthorization, &testKey.PublicKey)
		parsedAttObj, err := ParseAttestationObject(attObj)
		require.NoError(t, err)

		config := &AttestationValidationConfig{
			ValidateEKCertificate: false, // Disable EK validation for this test
			RequiredFormats:       []AttestationFormat{AttestationFormatTPM},
			AllowedTPMIdentifiers: []string{"TPM-ALLOWED-001"},
		}
		...
		require.NoError(t, err)
		require.NotNil(t, result)
		require.Equal(t, "TPM-ALLOWED-001", result.PermanentIdentifier)

		t.Log("✓ Allowed TPM passed validation")
	})

	// Test 2: Allowlist with non-allowed TPM should fail
	t.Run("NonAllowedTPMFails", func(t *testing.T) {
		attObj := tpm2.GenerateAttestation(t, keyAuthorization, &testKey.PublicKey)
		parsedAttObj, err := ParseAttestationObject(attObj)
		require.NoError(t, err)
		...
		_, err = validator.ValidateAttestation(context.Background(), parsedAttObj, keyAuthorization, config)
		require.Error(t, err)
		require.Contains(t, err.Error(), "not in the allowlist")

		t.Log("✓ Non-allowed TPM correctly rejected")
	})

	// Test 3: Empty allowlist should allow all TPMs
	t.Run("EmptyAllowlistAllowsAll", func(t *testing.T) {
		attObj := tpm2.GenerateAttestation(t, keyAuthorization, &testKey.PublicKey)
		parsedAttObj, err := ParseAttestationObject(attObj)
		require.NoError(t, err)
		...
		config := &AttestationValidationConfig{
			ValidateEKCertificate: false, // Disable EK validation for this test
			RequiredFormats:       []AttestationFormat{AttestationFormatTPM},
			AllowedTPMIdentifiers: []string{}, // Empty allowlist
		}
		...
		t.Log("✓ Empty allowlist correctly allows all TPMs")
	})

	// Test 4: Multiple entries in allowlist
	t.Run("MultipleAllowedTPMs", func(t *testing.T) {
		attObj := tpm2.GenerateAttestation(t, keyAuthorization, &testKey.PublicKey)
		parsedAttObj, err := ParseAttestationObject(attObj)
		require.NoError(t, err)
		...
		config := &AttestationValidationConfig{
			ValidateEKCertificate: false, // Disable EK validation for this test
			RequiredFormats:       []AttestationFormat{AttestationFormatTPM},
			AllowedTPMIdentifiers: []string{"TPM-ALLOWED-001", "TPM-NOT-ALLOWED-002", "TPM-ALLOWED-003"},
		}
		...
		result, err := validator.ValidateAttestation(context.Background(), parsedAttObj, keyAuthorization, config)
		require.NoError(t, err)
		require.Equal(t, "TPM-NOT-ALLOWED-002", result.PermanentIdentifier)

		t.Log("✓ Multiple allowlist entries work correctly")
	})
}

// TestTPMAttestationBlocklist tests TPM identifier blocklist functionality
func TestTPMAttestationBlocklist(t *testing.T) {
	tpm1 := NewSimulatedTPM(t, "TPM-GOOD-001", "TestManufacturer")
	tpm2 := NewSimulatedTPM(t, "TPM-BLOCKED-002", "TestManufacturer")
	...
	// Test 1: Blocklist should reject blocked TPM
	t.Run("BlockedTPMRejected", func(t *testing.T) {
		attObj := tpm2.GenerateAttestation(t, keyAuthorization, &testKey.PublicKey)
		parsedAttObj, err := ParseAttestationObject(attObj)
		require.NoError(t, err)
		...
		config := &AttestationValidationConfig{
			ValidateEKCertificate: false, // Disable EK validation for this test
			RequiredFormats:       []AttestationFormat{AttestationFormatTPM},
			BlockedTPMIdentifiers: []string{"TPM-BLOCKED-002"},
		}
		...
		_, err = validator.ValidateAttestation(context.Background(), parsedAttObj, keyAuthorization, config)
		require.Error(t, err)
		require.Contains(t, err.Error(), "is blocked")

		t.Log("✓ Blocked TPM correctly rejected")
	})

	// Test 2: Non-blocked TPM should succeed
	t.Run("NonBlockedTPMSucceeds", func(t *testing.T) {
		attObj := tpm1.GenerateAttestation(t, keyAuthorization, &testKey.PublicKey)
		parsedAttObj, err := ParseAttestationObject(attObj)
		require.NoError(t, err)
		...
		result, err := validator.ValidateAttestation(context.Background(), parsedAttObj, keyAuthorization, config)
		require.NoError(t, err)
		require.Equal(t, "TPM-GOOD-001", result.PermanentIdentifier)

		t.Log("✓ Non-blocked TPM passed validation")
	})

	// Test 3: Empty blocklist should allow all TPMs
	t.Run("EmptyBlocklistAllowsAll", func(t *testing.T) {
		attObj := tpm2.GenerateAttestation(t, keyAuthorization, &testKey.PublicKey)
		parsedAttObj, err := ParseAttestationObject(attObj)
		require.NoError(t, err)
		...
		t.Log("✓ Empty blocklist allows all TPMs")
	})
}

// TestTPMAttestationAllowlistAndBlocklist tests combined allowlist and blocklist
func TestTPMAttestationAllowlistAndBlocklist(t *testing.T) {
	tpm1 := NewSimulatedTPM(t, "TPM-ALLOWED-BUT-BLOCKED", "TestManufacturer")
	tpm2 := NewSimulatedTPM(t, "TPM-ALLOWED-NOT-BLOCKED", "TestManufacturer")
	tpm3 := NewSimulatedTPM(t, "TPM-NOT-ALLOWED", "TestManufacturer")
	...
	// Test 1: Blocklist takes precedence over allowlist
	t.Run("BlocklistTakesPrecedence", func(t *testing.T) {
		attObj := tpm1.GenerateAttestation(t, keyAuthorization, &testKey.PublicKey)
		parsedAttObj, err := ParseAttestationObject(attObj)
		require.NoError(t, err)
		...
		config := &AttestationValidationConfig{
			RequiredFormats:       []AttestationFormat{AttestationFormatTPM},
			AllowedTPMIdentifiers: []string{"TPM-ALLOWED-BUT-BLOCKED", "TPM-ALLOWED-NOT-BLOCKED"},
			BlockedTPMIdentifiers: []string{"TPM-ALLOWED-BUT-BLOCKED"},
		}
		...
		t.Log("✓ Blocklist correctly takes precedence over allowlist")
	})

	// Test 2: Allowed and not blocked should succeed
	t.Run("AllowedNotBlockedSucceeds", func(t *testing.T) {
		attObj := tpm2.GenerateAttestation(t, keyAuthorization, &testKey.PublicKey)
		parsedAttObj, err := ParseAttestationObject(attObj)
		require.NoError(t, err)
		...
		result, err := validator.ValidateAttestation(context.Background(), parsedAttObj, keyAuthorization, config)
		require.NoError(t, err)
		require.Equal(t, "TPM-ALLOWED-NOT-BLOCKED", result.PermanentIdentifier)

		t.Log("✓ TPM allowed and not blocked passed validation")
	})

	// Test 3: Not in allowlist should fail
	t.Run("NotInAllowlistFails", func(t *testing.T) {
		attObj := tpm3.GenerateAttestation(t, keyAuthorization, &testKey.PublicKey)
		parsedAttObj, err := ParseAttestationObject(attObj)
		require.NoError(t, err)
		...
		t.Log("✓ TPM not in allowlist correctly rejected")
	})
}
